...
def export_tmp(self, context):
    ...
    while(os.path.isdir(getExportPath(strip.sound_strip_additionals) +
          str(strip.sound_strip_additionals.al_tmpID) + "_data") or
          os.path.isfile(getExportPath(strip.sound_strip_additionals) +
          str(strip.sound_strip_additionals.al_tmpID) + ".aup") or
          os.path.isfile(bpy.app.tempdir+str(strip.sound_strip_additionals.al_tmpID) + ".wav")):
    ...
    if not filepath:
        try:
            # audProgram =
            subprocess.Popen(["audacity", audFilepath])
        except Exception:
            self.report(type={'ERROR'}, message=lang_error2)
    else:
        try:
            # audProgram =
            subprocess.Popen([filepath, audFilepath])
        except Exception:
            print(Exception)
            self.report(type={'ERROR'}, message=lang_error2)
...
def addStrip(self, context, strip, channel=None, frame_start=None):
    ...
    if strip.sound_strip_additionals.al_import_copy or copyImportRequired(strip):
        ...
        while os.path.isfile(os.path.join(dirname, name + '_EDIT' + "I"*edit + ext)):
            edit += 1

        dest_path = os.path.join(dirname, name + '_EDIT' + "I"*edit + ext)
...
def copyImportRequired(strip):
    ...
    """
    return (not strip.sound_strip_additionals.al_export == 'CUSTOM' and
            strip.sound_strip_additionals.al_import_fromsame)
...
def getImportPath(strip_additionals):
    ...
    if os.path.basename(src_path) == '':
        src_path = os.path.join(src_path, str(strip_additionals.al_tmpID)+".wav")
    return src_path
